...
module Rake
    ...
    class CProjectTaskLib < TaskLib
        ...
        def objfiles
            spec.sources.pathmap("#{intermediatedir}/%n.obj")
        end
        ...
        def define_compile_tasks
            ...
            spec.sources.zip(objfiles, depfiles).each do |srcfile, objfile, depfile|
                ...
                file objfile do
                    ...
                    case srcfile
                    when /\.cpp$/
                        sh "cl /nologo #{ENV["CFLAGS"]} #{ENV["CPPFLAGS"]} #{includes} /Tp\"#{srcfile}\" /Fo\"#{objfile}\""
                        #sh "clang++ #{ENV["CFLAGS"]} #{ENV["CPPFLAGS"]} #{includes} -c #{srcfile} -o #{objfile}"
                    when /\.c$/
                        sh "cl /nologo #{ENV["CFLAGS"]} #{includes} /Tc\"#{srcfile}\" /Fo\"#{objfile}\""
                        #sh "clang #{ENV["CFLAGS"]} #{includes} -c #{srcfile} -o #{objfile}"
                    else
    ...
    class StaticLibraryTask < CProjectTaskLib
        ...
        def define_link_task
            ...
            file filename => objfiles do
                sh "lib /nologo /out:\"#{filename}\" #{objfiles.join(" ")}"
            end
        end
    ...
    class ExecutableTask < CProjectTaskLib
        ...
        def define_link_task
            ...
            file executable => objfiles do
                sh "link /nologo #{ENV["LDFLAGS"]} /out:\"#{executable}\" #{libincludes} #{objfiles.join(" ")} #{libraries}"
                #sh "clang++ #{ENV["LDFLAGS"]} -o #{executable} #{libincludes} #{objfiles.join(" ")} #{libraries}"
            end
        end
